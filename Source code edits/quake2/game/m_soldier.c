...
// check for gib
	if (self->health <= self->gib_health)
	{
		...
		//spawn_heart(self, start, 10, 5);

		self->enemy = attacker;
		if (attacker && attacker->client){
			spawn_heart(self, attacker);
			int		index;
			gitem_t *item = FindItem("Hearts");
			if (item)
			{
				index = ITEM_INDEX(item);
				attacker->client->pers.inventory[index] += 10;
				if (attacker->client->pers.inventory[index] > attacker->client->pers.max_hearts)
					attacker->client->pers.inventory[index] = attacker->client->pers.max_hearts;
			}
